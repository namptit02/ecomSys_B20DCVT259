...
def add_to_cart(request):
    ...
    if product_type == PRODUCT_TYPE['BOOK']:
        products = call_api(request, 'http://127.0.0.1:8008/books')
        key = 'books'
    ...
    if validator.validate(item):
        # try:
            ...
            exist_item = item_test.filter(product_price = product['price'])
            exist_item_with_old_price = item_test.exclude(product_price = product['price'])

            if exist_item_with_old_price :
                exist_item_with_old_price[0].product_status = PRODUCT_STATUS['EXPIRED']
                exist_item_with_old_price[0].save()

            if exist_item :
                exist_item[0].quantity += quantity
                exist_item[0].save()
            else:
